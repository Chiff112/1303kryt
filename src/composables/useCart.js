...
/**
 * useCart — a shared cart + favorites "store" built on Vue reactivity.
 *
 * State lives at module scope, so every component that calls useCart()
 * reads and writes the SAME data (header summary, popup, cart page,
 * product cards, footer all stay in sync).
 *
 * Includes:
 *   - cart items with quantities
 *   - favorites (heart toggle)
 *   - promo code (15% off orders >= 1500 ₽, applied once)
 *   - derived totals: count, price, bonus, delivery, grand total
 */
...
// subtotal = sum of item prices (before promo / delivery)
...
// totalPrice = subtotal after discount (what the cart/order shows)
...
// Delivery is free over 1000 ₽, otherwise 90 ₽ (only when cart not empty)
